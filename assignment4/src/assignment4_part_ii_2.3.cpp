...
void mm_kernel(double* a, double* b, double* c, int N, int numTeams, int numThreads) {
    ...
    //changed schedule(static,1) to schedule(static), which gives best performance
    #pragma omp distribute parallel for schedule(static) collapse(2) num_threads(numThreads)
    for( long i = 0; i < N; ++i ) {
        for( long j = 0; j < N; ++j ) {
            for( int k = 0; k < N; ++k ) {
...
double mm(int N, int REP, double expected_result, int numTeams, int numThreads) {
  ...
  for(long i = 0; i < N; ++i) {
      for(int j = 0; j < N; ++j) {
        ...
        /*
        * TODO : Part 2.2 : Change initializations here to apply column and row major storage as asked in task sheet
        */
        b[i*N + j] = (i*j)/(N*50); 
        //used variant 2 where (c) array is column major
        c[j*N + i] = (i*j)/(N*50);  
      }
  }
...
int main(int argc, char* argv[]) {
  ...
  /*
    * TODO: Part 2.3: You can adjust thread and team numbers by passing argv[1] and argv[2] to program
    */ 
  char *pEnd;
  long numTeams=0;
  long numThreads=0;
