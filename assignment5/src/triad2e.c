...
int main(int argc, char * argv[]) {
    ...
    long datasetSize = N;
    double m_flop = 0;
    //int threads;  make global
    long cycles;//=10;

    // TASK 1.a
    //while (datasetSize <= N && cycles > 1)
    //{
        cycles = REP; //no division by  datasetSize anymore ;

        time_a = triad(datasetSize, cycles, &threads);
        m_flop = 2.0 * (double)datasetSize * (double)(cycles/threads) * 1.0e-6;
        performance_a = m_flop / time_a;

        printf("| %12ld | %12d | %12.2f | %12ld |\n", datasetSize, threads, performance_a, cycles/threads);
