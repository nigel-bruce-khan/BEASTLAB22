   ...
   // #pragma omp parallel for schedule(static) num_threads(128)
    for (unsigned int y = 1; y < _nyCoarseGrid + 1; y++)
    {
      for (unsigned int x = 1; x < _nxCoarseGrid + 1; x++)
      {
        ...
        *(coarseRhs + coarsePos) = *(fineRhs+finePos) - (*(fineVal_W+finePos) - 2.0 * (*(fineVal_C+finePos)) + (*(fineVal_E+finePos))) / _hxFineGrid2 - (*(fineVal_S+finePos) - 2.0 * (*(fineVal_C+finePos)) + *(fineVal_N+finePos)) / _hyFineGrid2;
