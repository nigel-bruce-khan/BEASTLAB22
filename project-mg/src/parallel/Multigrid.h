...
public:
  ...
  Multigrid(const unsigned int nxFine, const unsigned int nyFine)
      : _callRecursion((nxFine > 2) && (nyFine > 2)),
        ...
        _coarseGridValues1(_callRecursion ? new (std::align_val_t(32)) FLOAT[(_nxCoarseGrid + 2) * (_nyCoarseGrid + 2)] : NULL),
        _coarseGridValues2(_callRecursion ? new (std::align_val_t(32)) FLOAT[(_nxCoarseGrid + 2) * (_nyCoarseGrid + 2)] : NULL),
        _coarseGridRhs(_callRecursion ? new (std::align_val_t(32)) FLOAT[(_nxCoarseGrid + 2) * (_nyCoarseGrid + 2)] : NULL),
        _multigrid(_callRecursion ? new (std::align_val_t(32)) Multigrid(_nxCoarseGrid, _nyCoarseGrid) : NULL),
        _jacobi(nxFine, nyFine),
        _restriction(_nxCoarseGrid, _nyCoarseGrid),
        _prolongation(_nxCoarseGrid, _nyCoarseGrid), 
        _NUM_THREADS((nxFine < 16) ? 1 : 128)
  {
