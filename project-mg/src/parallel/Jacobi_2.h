  ...
  void iterate(const FLOAT *const readField, FLOAT *const writeField, const FLOAT *const rhs) const
  {
    ...
    for (unsigned int y = 1; y < _ny + 1; y++)
    {
        ...
        for (unsigned int x = 1; x < ub + 1; x+=4)
        {
          ...
          pos = (y-1)*(_nx+2) + x-1;
          __m256d read_s = _mm256_loadu_pd(readPtr_S + pos);
          __m256d read_w = _mm256_loadu_pd(readPtr_W  + pos);
          __m256d read_e = _mm256_loadu_pd(readPtr_E + pos);
          __m256d read_n = _mm256_loadu_pd(readPtr_N + pos); //Cache miss HS
          __m256d rhsptr = _mm256_loadu_pd(rhsPtr + pos); //Cache miss HS

          __m256d a = _mm256_mul_pd(_rhs, rhsptr);
          __m256d b = _mm256_add_pd(read_w, read_e);
          __m256d c = _mm256_add_pd(read_s, read_n); //Cache miss HS
          __m256d d = _mm256_mul_pd(_x, b);
          __m256d e = _mm256_mul_pd(_y, c); //Cache miss HS
          __m256d f = _mm256_add_pd(a, d); //Cache miss HS
          __m256d g = _mm256_add_pd(f, e); //Cache miss HS
          _mm256_store_pd(buffer, g); //Cache miss HS
          for (int q = 0; q < 4; q++) {
	       	    writePtr[pos+q] = buffer[q];//Cache miss HS
	        }
        }
