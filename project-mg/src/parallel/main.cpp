...
int main(int argc, char *argv[])
{
  ...
  FLOAT *field1 = new (std::align_val_t(32)) FLOAT[(nx + 2) * (nx + 2)];
  if (field1 == NULL)
  {
    std::cout << "ERROR field1==NULL!" << std::endl;
    return -1;
  }
  FLOAT *field2 = new (std::align_val_t(32)) FLOAT[(nx + 2) * (nx + 2)];
  if (field2 == NULL)
  {
    std::cout << "ERROR field2==NULL!" << std::endl;
    return -1;
  }
  FLOAT *rhs = new (std::align_val_t(32)) FLOAT[(nx + 2) * (nx + 2)];
  if (rhs == NULL)
  {
